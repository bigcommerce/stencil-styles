...
module.exports = function StencilStyles(options) {
    ...
    self.compileCss = function(compiler, options, callback) {
        switch(compiler) {
            case 'scss':
                self.scssCompiler(options, doneCompiling);
                break;
        }
    ...
    self.scssCompiler = function(options, callback) {
        ...
        sassFunctions['stencilFontFamily($name)'] = function(nameObj) {
            return self.stencilFont(options, nameObj.getValue(), 'family');
        };

        sassFunctions['stencilFontWeight($name)'] = function(nameObj) {
            return self.stencilFont(options, nameObj.getValue(), 'weight');
        };
    ...
    self.scssImporter = function(url, prev) {
        var prevParts = Path.parse(prev),
            urlParts = Path.parse(url),
            fullUrls = internals.fullUrls,
        ...
        if (options.files[fullUrl] === undefined) {
            ...
            return {
                file: fullUrl,
                contents: options.files[fullUrl]
            };
        }
    };

    self.stencilFont = function(options, key, type) {
        var value = options.themeSettings[key],
