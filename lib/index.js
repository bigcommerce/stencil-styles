...
internals.scssCompiler = function (options, callback) {
     ...
     * Eg: "Open Sans_700"
     * @param nameObj - id to look for in the config
     * @param type - 'family' or 'weight'
     * @returns {*}
     */
    function stencilFont(nameObj, type) {
        ...
        if (_.isEmpty(pair[index])) {
            return Sass.NULL;
        }

        return new Sass.types.String(pair[index]);
    }

    sassFunctions['stencilFontFamily($name)'] = function(nameObj) {
        return stencilFont(nameObj, 'family');
    };

    sassFunctions['stencilFontWeight($name)'] = function(nameObj) {
        return stencilFont(nameObj, 'weight');
    };
