...
const sassLegacy = require('@bigcommerce/node-sass');
...
function StencilStyles() {
    ...
    this.scssCompiler = (options, callback) => {
        console.error("********* Node sass version from stencil styles *********", [options.version]);
        switch(options.version) {
        case 'sass_v4':
            this.scssRender(options, callback);
            break;
        default:
            this.legacyScssRender(options, callback);
            break;
        }
    };
    ...
    this.scssFunctions = (themeSettings) => {
        ...
        sassFunctions['stencilNumber($name, $unit: px)'] = (nameObj, unitObj) => {
            ...
            return new sassLegacy.types.Number(value, unit);
        };
        ...
        sassFunctions['stencilColor($name)'] = (nameObj) => {
            ...
            if (themeSettings[name]) {
                ...
                ret = new sassLegacy.types.Color(parseInt('0xff' + val, 16));
            } else {
                ret = sassLegacy.NULL;
            }
        ...
        sassFunctions['stencilString($name)'] = (nameObj) => {
            ...
            return themeSettings[name]
                ? new sassLegacy.types.String(themeSettings[name])
                : sassLegacy.NULL;
        };
        ...
        sassFunctions['stencilImage($image, $size)'] = (imageObj, sizeObj) => {
            ...
            if (themeSettings[image].indexOf('{:size}') !== -1 && sizeRegex.test(themeSettings[size])) {
                ret = new sassLegacy.types.String(themeSettings[image].replace('{:size}', themeSettings[size]));
            } else {
                ret = sassLegacy.NULL;
            }
    ...
    this.defaultFontParser = (value, type) => {
        ...
        return sassLegacy.types.String(formattedString);
    };
    ...
    this.legacyScssRender = (options, callback) => {
        ...
        try {
            sassLegacy.render(params, (err, result) => {
                if (err) {
                    return callback(err);
                }
                console.error("********* Node sass version from stencil styles: legacyScssRender *********");
    ...
    this.scssRender = (options, callback) => {
        ...
        try {
            sass.render(params, (err, result) => {
                ...
                console.error("********* Node sass version from stencil styles: scssRender *********");
                callback(null, result.css);
            });
        }
        catch (e) {
